 ...
 *           El retiro (Slider de Sacrificio) vive en pantalla propia, GoalsTransferScreen — no acá.
...
export function GoalsScreen() {
  ...
  const {
    goals, isAddVisible, selectedGoal, poolTotal, disponible,
  ...
  return (
    <View style={styles.screen}>
      <Animated.ScrollView
        style={styles.scroll}
        contentContainerStyle={[styles.content, { paddingTop: headerHeight, paddingBottom: insets.bottom + spacing.stackLg * 2 }]}
      ...
      >
        <GoalsSummaryCard poolTotal={poolTotal} disponible={disponible} onAdd={handleAnadir} />
        ...
        <View style={styles.cardList}>
          {goals.map((goal) => (
            <GoalCard key={goal.id} goal={goal} onPress={handleCardPress} onDeposit={setDepositGoal} />
          ))}
        </View>
      </Animated.ScrollView>
